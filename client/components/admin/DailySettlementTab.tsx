    ...
    return (
        <div className="space-y-6 md:space-y-8 animate-fade-in-up">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-white p-6 rounded-[32px] border border-gray-100 shadow-sm flex flex-col justify-between hover:border-bee-yellow hover:shadow-xl transition-all group/stat relative overflow-hidden">
                    ...
                    <div className="relative">
                        <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest mb-3 flex items-center gap-1.5">
                            <i className="fa-solid fa-cash-register opacity-50"></i> {t.admin?.overview?.revenue || '총 매출액'}
                        </p>
                        <h3 className="text-2xl font-black tabular-nums text-bee-black">₩{dailySettlementStats.totalRevenue.toLocaleString()}</h3>
                    </div>
                    <div className="mt-4 flex gap-2 items-center">
                        <div className="px-2 py-1 bg-bee-yellow/10 rounded-lg text-[9px] font-black text-bee-yellow flex items-center gap-1">
                            <i className="fa-solid fa-truck text-[7px]"></i> {dailySettlementStats.deliveryCount}
                        </div>
                        <div className="px-2 py-1 bg-bee-blue/10 rounded-lg text-[9px] font-black text-bee-blue flex items-center gap-1">
                            <i className="fa-solid fa-box-archive text-[7px]"></i> {dailySettlementStats.storageCount}
                        </div>
                    </div>
                </div>

                <div className="bg-white p-6 rounded-[32px] border border-gray-100 shadow-sm flex flex-col justify-between hover:border-emerald-400 hover:shadow-xl transition-all group/stat relative overflow-hidden">
                    <div className="relative">
                        <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest mb-3 flex items-center gap-1.5">
                            <i className="fa-solid fa-check-double text-emerald-500"></i> {t.admin?.logistics?.filter_completed || '정산 확정'}
                        </p>
                        <h3 className="text-2xl font-black tabular-nums text-emerald-500">₩{(dailySettlementStats.confirmedAmount || 0).toLocaleString()}</h3>
                    </div>
                    <div className="mt-4 flex items-center justify-between">
                        <p className="text-[9px] font-black text-gray-400 uppercase">Pending: ₩{(dailySettlementStats.unconfirmedAmount || 0).toLocaleString()}</p>
                        <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></div>
                    </div>
                </div>

                <div className="bg-white p-6 rounded-[32px] border border-gray-100 shadow-sm flex flex-col justify-between hover:border-orange-400 hover:shadow-xl transition-all group/stat relative overflow-hidden">
                    <div className="relative">
                        <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest mb-3 flex items-center gap-1.5">
                            <i className="fa-solid fa-hand-holding-dollar text-orange-400"></i> Payouts
                        </p>
                        <h3 className="text-2xl font-black tabular-nums text-orange-500">₩{(dailySettlementStats.partnerPayoutTotal || 0).toLocaleString()}</h3>
                    </div>
                    <div className="mt-4">
                        <p className="text-[9px] font-black text-orange-300 uppercase tracking-tight">파트너/지점 지급 정산액</p>
                    </div>
                </div>

                <div className="bg-bee-black p-6 rounded-[32px] shadow-2xl flex flex-col justify-between relative overflow-hidden group/card hover:-translate-y-1 transition-all">
                    ...
                    <div className="relative z-10">
                        <div className="flex items-center justify-between mb-3">
                            <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">순수익 (Net Profit)</p>
                            <span className="w-6 h-6 rounded-full bg-bee-yellow/10 flex items-center justify-center">
                                <i className="fa-solid fa-crown text-[10px] text-bee-yellow"></i>
                            </span>
                        </div>
                        <h3 className="text-2xl font-black italic tabular-nums text-bee-yellow drop-shadow-lg">
                            ₩{(dailySettlementStats.netProfit - (dailySettlementStats.partnerPayoutTotal || 0)).toLocaleString()}
                        </h3>
                    </div>
                </div>
            </div>
            ...
            {/* [스봉이] 취소/환불 별도 표기 섹션 💅✨ */}
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-red-50/50 p-6 rounded-[32px] border border-red-100 flex flex-col justify-between group/stat">
                    <div>
                        <p className="text-[10px] font-black text-red-400 uppercase tracking-widest mb-2 flex items-center gap-1.5">
                            <i className="fa-solid fa-ban"></i> 취소 금액 (Cancelled)
                        </p>
                        <h3 className="text-xl font-black tabular-nums text-red-500">
                            ₩{(dailySettlementStats.cancelledTotal || 0).toLocaleString()}
                        </h3>
                    </div>
                    <p className="text-[9px] font-bold text-red-300 mt-3">{dailySettlementStats.cancelledCount || 0}건의 예약 취소</p>
                </div>

                <div className="bg-orange-50/50 p-6 rounded-[32px] border border-orange-100 flex flex-col justify-between group/stat">
                    <div>
                        <p className="text-[10px] font-black text-orange-400 uppercase tracking-widest mb-2 flex items-center gap-1.5">
                            <i className="fa-solid fa-rotate-left"></i> 환불 금액 (Refunded)
                        </p>
                        <h3 className="text-xl font-black tabular-nums text-orange-500">
                            ₩{(dailySettlementStats.refundedTotal || 0).toLocaleString()}
                        </h3>
                    </div>
                    <p className="text-[9px] font-bold text-orange-300 mt-3">{dailySettlementStats.refundedCount || 0}건의 환불 처리</p>
                </div>
                                        ...
                                        <tr key={b.id} className={`hover:bg-gray-50/50 transition-all group/row cursor-pointer ${isCancelled ? 'bg-gray-50/80 opacity-60' : ''}`} onClick={() => setSelectedBooking(b)}>
                                            <td className="px-6 py-5">
                                                <div className="flex flex-col gap-1.5">
                                                    <span className={`w-fit px-2 py-0.5 rounded-md text-[8px] font-black uppercase ${
                                                        isCancelled ? 'bg-red-50 text-red-400 border border-red-100' :
                                                        b.status === BookingStatus.COMPLETED ? 'bg-bee-black text-bee-yellow' : 'bg-gray-100 text-gray-400'
                                                    }`}>
                                                        {b.status}
                                                    </span>
                                            ...
                                            <td className="px-6 py-5">
                                                <div className="flex justify-center">
                                                    <div className={`text-[9px] font-black px-3 py-1.5 rounded-full border shadow-sm flex items-center gap-1.5 transition-all ${
                                                        isCancelled ? 'bg-white border-dashed border-gray-200 text-gray-300' :
                                                        b.settlementStatus === 'CONFIRMED' 
                                                        ? 'bg-emerald-50 border-emerald-100 text-emerald-600' 
                                                        : 'bg-white border-gray-100 text-gray-300 group-hover/row:border-bee-yellow'
                                                    }`}>
                ...
                <div className="space-y-6 md:space-y-8 h-fit">
                    ...
                    <div className="bg-white p-6 md:p-8 rounded-[40px] border border-gray-100 shadow-sm space-y-8 relative overflow-hidden group/cash">
                        ...
                        <div className="bg-gray-50/50 rounded-[32px] border border-gray-100 overflow-hidden divide-y divide-gray-100 group/ledger">
                            <div className="p-6 grid grid-cols-2 gap-10 relative bg-white">
                            ...
                            <div className={`p-6 flex justify-between items-center transition-all duration-700 ${dailySettlementStats.revenueByMethod.cash - (cashClosing.actualCash || 0) === 0
                                ? 'bg-emerald-50/50'
                                : 'bg-orange-50/50'
                                }`}>
                                <div className="flex flex-col">
                                    <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1 items-center flex gap-2">
                                        마감 차액 분석
                                        {dailySettlementStats.revenueByMethod.cash - (cashClosing.actualCash || 0) === 0 && (
                                            <span className="text-[8px] px-2 py-0.5 bg-emerald-500 text-white rounded-full animate-bounce">일치함</span>
                                        )}
                                    </span>
                                    <span className={`text-3xl font-black italic font-mono tracking-tighter ${dailySettlementStats.revenueByMethod.cash - (cashClosing.actualCash || 0) !== 0 ? 'text-orange-500' : 'text-emerald-500'}`}>
                                        ₩{(dailySettlementStats.revenueByMethod.cash - (cashClosing.actualCash || 0)).toLocaleString()}
                                    </span>
                                </div>
                                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-500 ${dailySettlementStats.revenueByMethod.cash - (cashClosing.actualCash || 0) === 0 
                                    ? 'bg-emerald-500 text-white' 
                                    : 'bg-orange-500 text-white animate-pulse shadow-lg shadow-orange-200'
                                    }`}>
                                    <i className={`fa-solid ${dailySettlementStats.revenueByMethod.cash - (cashClosing.actualCash || 0) === 0 ? 'fa-check-double' : 'fa-triangle-exclamation'} text-xl`}></i>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex gap-2">
                                ...
                                <button
                                    ...
                                    className="px-6 bg-orange-50 text-orange-500 font-black rounded-2xl hover:bg-orange-500 hover:text-white transition-all shadow-sm border border-orange-100"
                                >
                                    추가
                                </button>
                            </div>
