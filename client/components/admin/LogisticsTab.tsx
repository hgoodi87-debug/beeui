...
const LogisticsTab: React.FC<LogisticsTabProps> = ({
    ...
    return (
        <div className="space-y-6 md:space-y-8 animate-fade-in-up">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-2">
                    ...
                    <div className="flex bg-white p-1.5 rounded-2xl shadow-sm border border-gray-100 w-fit">
                        {(['ALL', 'PENDING', 'ACTIVE', 'COMPLETED'] as const).map(tab => (
                            <button
                                ...
                                className={`px-5 py-2.5 rounded-xl text-[10px] font-black transition-all ${activeStatusTab === tab ? 'bg-bee-black text-bee-yellow shadow-md' : 'text-gray-400 hover:bg-gray-50'}`}
                            >
                ...
                <div className="flex-1 flex justify-start lg:justify-end">
                    {onAddManual && (
                        <button
                            onClick={onAddManual}
                            className="px-6 py-3 bg-bee-yellow text-bee-black font-black rounded-2xl text-[11px] shadow-xl shadow-bee-yellow/20 hover:scale-[1.02] active:scale-95 transition-all flex items-center gap-2 border-2 border-bee-yellow hover:bg-bee-black hover:text-bee-yellow hover:border-bee-black"
                        >
            ...
            <div className="bg-white p-2 md:p-10 rounded-[40px] shadow-sm border border-gray-100 overflow-hidden">
                                ...
                                <tr key={b.id} className="group hover:bg-gray-50/50 transition-colors">
                                    <td className="px-6 py-5">
                                        <div className="flex flex-col">
                                            <span className="text-[9px] font-black text-gray-300 uppercase tracking-widest mb-1">{b.id}</span>
                                            <span className="font-black text-bee-black group-hover:text-blue-600 transition-colors cursor-pointer" onClick={() => { setSelectedBooking({ ...b }); }}>{b.userName}</span>
                                    ...
                                    <td className="px-6 py-5">
                                        {b.serviceType === ServiceType.DELIVERY ? (
                                            <>
                                                <div className="text-[11px] font-bold text-gray-600">{b.pickupDate}</div>
                                    ...
                                    <td className="px-6 py-5">
                                        ...
                                        <div className="text-sm font-black text-bee-black tracking-tight">₩{(b.finalPrice || 0).toLocaleString()}</div>
                                    </td>
                                    <td className="px-6 py-5">
                                        <select
                                            value={b.status}
                ...
                <div className="md:hidden space-y-4">
                    {filteredBookings.length > 0 ? filteredBookings.map(b => (
                        <div key={b.id} className="bg-white p-5 rounded-[24px] border border-gray-100 shadow-sm flex flex-col gap-4">
                            <div className="flex justify-between items-start">
                                <div>
                                    ...
                                    <h4 className="font-black text-lg text-bee-black">{b.userName}</h4>
                                </div>
                                <select
                                    value={b.status}
                                    onChange={e => updateStatus(b.id!, e.target.value as BookingStatus)}
                            ...
                            <div className="flex items-center justify-between border-t border-gray-50 pt-3">
                                <div className="flex gap-2">
                                    ...
                                    <button onClick={() => handlePrintLabel(b)} className="w-8 h-8 rounded-full bg-gray-100 text-gray-500 flex items-center justify-center">
                        ...
                        <div className="bg-white p-10 rounded-[24px] text-center border border-gray-100">
