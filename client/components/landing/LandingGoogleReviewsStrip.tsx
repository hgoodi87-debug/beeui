...
const LandingGoogleReviewsStrip: React.FC = () => {
        ...
        (async () => {
            try {
                const [revs, sums] = await Promise.all([
                    supabaseGet<GoogleReview[]>('google_reviews?select=*&is_visible=eq.true&order=rating.desc&limit=10'),
    ...
    const renderCard = (review: GoogleReview, key: string) => (
        <div
            key={key}
            className="w-[280px] md:w-[320px] shrink-0 bg-white rounded-2xl p-4 md:p-5 shadow-lg border border-gray-100"
        >
            <div className="flex items-center gap-3 mb-2">
                {review.author_photo_url ? (
                    <img src={review.author_photo_url} alt="" className="w-8 h-8 rounded-full" />
                ) : (
