...
| **캐리어** (carrier) | ₩5,000 | ₩1,000 | ₩10,000 | ₩8,000 | ₩58,000 |
...
| 4시간 1분 ~ 5시간 | 5 | min(5000 + 1×1000, 10000) | ₩6,000 |
| 6시간 | 6 | min(5000 + 2×1000, 10000) | ₩7,000 |
| 7시간 | 7 | min(5000 + 3×1000, 10000) | ₩8,000 |
| **8시간** | 8 | day1 상한 적용 | **₩10,000** |
...
| 캐리어 | ₩5,000 | ₩1,000 | **8시간** (정책상 8시간부터 day1 적용) |
...
| 2026-04-07 | `STORAGE_RATES.hourlyAfter4h` 단가 수정: 손가방 200→1000, 캐리어 250→1000, 유모차 200→2500. `getSingleBagStoragePrice` 공식에 `day1` 상한 처리(`Math.min`) 추가. |
| 2026-04-08 | 캐리어 4시간 초과 시간당 단가를 최종 정책 기준 1,000원으로 조정. |
