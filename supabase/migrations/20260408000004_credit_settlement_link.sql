-- W-8: credit_used 컬럼 추가 + 의존 뷰 재생성
-- booking_details에 credit_used 컬럼 추가
-- admin_booking_list_v1 → admin_revenue_daily_v1 → admin_revenue_monthly_v1 CASCADE 재생성
...
-- 3) admin_booking_list_v1 재생성 (credit_used, insurance_fee, payout_id 포함)
...
WITH booking_core AS (
  SELECT
    ...
    COALESCE(baggage.bag_sizes, jsonb_build_object('handBag',0,'carrier',0,'strollerBicycle',0)) AS bag_sizes,
    bd.insurance_level,
  ...
  JOIN reservations r ON r.id = bd.reservation_id
...
-- 5) admin_revenue_monthly_v1 재생성
