...
--
-- 사전 준비:
--   1) Supabase Dashboard → Project Settings → Secrets 에 CRON_SECRET 등록
--   2) Database 설정에 app.cron_secret 동일 값 등록
...
-- app.cron_secret 세션 변수 설정 (실제 값으로 교체 필요)
-- ALTER DATABASE postgres SET "app.cron_secret" = '<YOUR_CRON_SECRET>';
-- 동시에 Supabase Dashboard → Project Settings → Secrets 에서
--   CRON_SECRET = <동일한 값> 등록 필요
