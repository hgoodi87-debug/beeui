-- Fix: admin_booking_list_v1 INNER JOIN → LEFT JOIN
-- booking_details.reservation_id = NULL인 신규 예약도 뷰에 포함되도록 수정
-- 기존: JOIN reservations r → reservation_id 없으면 안 보임 (30건 누락)
-- 수정: LEFT JOIN reservations r + NULL 필드 기본값 처리
...
WITH booking_core AS (
  ...
  -- reservation 없는 신규 예약의 branch 정보: pickup location → branch
