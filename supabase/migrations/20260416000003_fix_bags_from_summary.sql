-- bags 컬럼이 0이지만 bag_summary에 내용이 있는 기존 예약 복구
-- bag_summary 예시: "핸드백 1개, 캐리어 2개, 유모차/자전거 1개"
-- → 숫자만 뽑아서 합산

UPDATE public.booking_details
SET bags = (
    SELECT COALESCE(SUM(n::integer), 0)
    FROM (
        SELECT unnest(regexp_matches(bag_summary, '(\d+)개', 'g')) AS n
    ) t
)
WHERE bags = 0
  AND bag_summary IS NOT NULL
  AND bag_summary <> '';

-- admin_booking_list_v1 뷰 재생성:
-- r.id 없는 예약의 bag_sizes를 bag_summary 파싱으로 채움
-- (기존 bag_sizes 기본값 {0,0,0} → 실제 값 반영)
...
WITH booking_core AS (
  SELECT
    ...
    bd.return_date,
    bd.return_time,
    -- bags: 예약 연결 있으면 reservation_items 합산, 없으면 bd.bags (4/6 이후 직접 저장값)
    COALESCE(
      CASE WHEN r.id IS NOT NULL THEN baggage.bag_count ELSE NULL END,
      bd.bags,
      0
    )::integer AS bags,
    -- bag_sizes: 예약 연결 있으면 reservation_items 분류, 없으면 bag_summary 파싱으로 복구
    COALESCE(
