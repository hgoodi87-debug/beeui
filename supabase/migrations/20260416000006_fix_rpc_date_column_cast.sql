-- ─────────────────────────────────────────────────────────────────────────────
-- Migration: 20260416000006_fix_rpc_date_column_cast.sql
-- Purpose:   admin_update_booking_details RPC date 컬럼 타입 캐스트 수정
--
-- 문제: pickup_date / dropoff_date / return_date 컬럼이 date 타입인데
--       p_payload->>'...' 는 text를 반환해서
--       COALESCE(text, date) → "types text and date cannot be matched" 400 에러
--
-- 해결: 해당 필드에 ::date 캐스트 추가
-- ─────────────────────────────────────────────────────────────────────────────
