-- ─────────────────────────────────────────────────────────────────────────────
-- Migration: 20260416000009_fix_view_ops_status_coalesce.sql
-- Purpose:   admin_booking_list_v1 뷰의 ops_status COALESCE 복구
--
-- 문제: 20260416000003 마이그레이션에서 뷰를 재생성할 때
--       r.ops_status만 사용 → bd.ops_status(booking_details 직접 기입값) 무시됨
--       → 상태 변경이 뷰에 반영 안 됨 → UI 상태가 원래 값으로 돌아옴
--       → 완료/보관중 탭에서 예약이 "사라지는" 현상 발생
--
-- 해결: COALESCE(r.ops_status, bd.ops_status) 복구
--       (20260409000002_booking_details_ops_status.sql의 올바른 로직 재적용)
-- ─────────────────────────────────────────────────────────────────────────────
...
WITH booking_core AS (
  SELECT
    ...
    -- ops_status: reservation 우선, 없으면 booking_details 직접 기입값 사용 (핵심 수정)
    ...
    bd.return_date,
    bd.return_time,
    -- bags: 예약 연결 있으면 reservation_items 합산, 없으면 bd.bags
    COALESCE(
      CASE WHEN r.id IS NOT NULL THEN baggage.bag_count ELSE NULL END,
      bd.bags,
      0
    )::integer AS bags,
    -- bag_sizes: 예약 연결 있으면 reservation_items 분류, 없으면 bag_summary 파싱
    COALESCE(
